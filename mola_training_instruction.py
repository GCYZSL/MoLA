...
def train(
    ...
    data_path: str = "./meta_moe_50k.hf",
    output_dir: str = "./meta_moe_llm2_3ep_2468",
    ...
    load_in_8bit: bool = False,  # if False, masks out inputs in loss
    # wandb params
    wandb_project: str = "",
    wandb_run_name: str = "",
    ...
    prompt_template_name_meta: str = "meta",  # The prompt template to use, will default to alpaca.
    obalance: bool = True,
):
    ...
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
        print(
            ...
            f"prompt template meta: {prompt_template_name_meta}\n"
        )
    assert (
        base_model
    ...
    prompter_meta = Prompter(prompt_template_name_meta)
    ...
    def generate_and_tokenize_prompt_meta(data_point):
        full_prompt = prompter_meta.generate_prompt_meta(
        ...
        if not train_on_inputs:
            user_prompt = prompter_meta.generate_prompt_meta(
    ...
    if val_set_size > 0:
        ...
        train_data = (
            ...
            #     pd.DataFrame(data=data["train"][:1000])).shuffle().map(generate_and_tokenize_prompt_meta)
            train_val["train"].shuffle().map(generate_and_tokenize_prompt_meta)
        )
        val_data = (
            train_val["test"].shuffle().map(generate_and_tokenize_prompt_meta)
        )
    else:
        train_data = data["train"].shuffle().map(generate_and_tokenize_prompt_meta)
        val_data = None
